...
public class ProductRepository : BaseRepository<Product>, IProductRepository
{
    ...
    public async Task<(IEnumerable<Product> Products, int TotalCount)> SearchAsync(ProductFilterVM model)
    {
        ...
        var products = await query.Skip((model.PageIndex - 1) * model.PageSize).Take(model.PageSize).ToListAsync();
